...
Кнопка `Search` выполняет поиск по IP из поля ввода. Кнопка `Refresh` заново определяет текущий публичный IP и обновляет данные о локации во время работы приложения.
